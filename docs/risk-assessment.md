...
| R19 | CI | TSan cannot instrument the coroutine context switch (asm/ucontext backend) — crashes `test_coroutine` at the infra level (`can't find longjmp buf`); ThreadPoolTests also report deliberate test-code races; the pool timer thread (2026-08-21) has no TSan coverage either | High | Medium | **High** — :unstable: (2026-08-18): TSan job stays `continue-on-error`; ASan/UBSan are the hard gates; coroutine coverage relies on ASan/UBSan + valgrind instead — the timer thread's races are serialized by `timer_lock` and verified under those same hard gates (see R21) |
