...
static void test_guard_page(void)
{
    ...
    ASSERT(loom_coro_create(coro_task, &counter, 4096, &coro) == LOOMWORKS_CORO_OK,
           "create with guard pages");
    if (coro) {
...
static void test_stress_yield_coroutines(void)
{
    ...
    for (int i = 0; i < N; i++) {
        loom_coroutine_t *coro = NULL;
        ASSERT(loom_coro_create(yield_coro_task, &counter, 0, &coro) == LOOMWORKS_CORO_OK,
               "create yield coro");
...
static void test_large_stack_coroutines(void)
{
    ...
    for (int i = 0; i < N; i++) {
        loom_coroutine_t *coro = NULL;
        ASSERT(loom_coro_create(coro_task, &counter, 262144, &coro) == LOOMWORKS_CORO_OK,
               "create 256KB stack coro");
